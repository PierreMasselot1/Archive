...
#define LOOP_XYZI(VAR) LOOP_S_L_N(VAR, A_AXIS, XYZI)
...
template<typename T> struct XYZIval;
template<typename T> struct XYZIEval;

typedef struct XYval<bool>          xy_bool_t;
typedef struct XYZval<bool>        xyz_bool_t;
typedef struct XYZEval<bool>      xyze_bool_t;
typedef struct XYZIval<bool>      xyzi_bool_t;
typedef struct XYZIEval<bool>      xyzie_bool_t;

typedef struct XYval<char>          xy_char_t;
typedef struct XYZval<char>        xyz_char_t;
typedef struct XYZEval<char>      xyze_char_t;
typedef struct XYZIEval<char>      xyzie_char_t;
typedef struct XYZIval<char>      xyzi_char_t;
...
struct XYZIval {
  ...
  FI T magnitude()                                      const { return (T)sqrtf(x*x + y*y + z*z + i*i); }
  FI operator T* ()                                           { return pos; }
  FI operator bool()                                          { return i || z || x || y; }
  FI void set(const T px)                                     { x = px;                                        }
  FI void set(const T px, const T py)                         { x = px;     y = py;                            }
  FI void set(const T px, const T py, const T pz)             { x = px;     y = py;     z = pz;                }
  ...
  FI void set(const XYval<T> pxy, const XYval<T> pzi)         { x = pxy.x;  y = pxy.y;  z = pzi.z;  i = pzi.i; }
  FI void set(const XYZval<T> pxyz, const T pi)               { x = pxyz.x; y = pxyz.y; z = pxyz.z; i = pi;    }
  FI void set(const T (&arr)[XY])                             { x = arr[0]; y = arr[1]; }
  FI void set(const T (&arr)[XYZ])                            { x = arr[0]; y = arr[1]; z = arr[2]; }
  FI void set(const T (&arr)[XYZI])                           { x = arr[0]; y = arr[1]; z = arr[2]; i = arr[3]; }
  #if XYZI_N > XYZI
    FI void set(const T (&arr)[XYZI_N])                       { x = arr[0]; y = arr[1]; z = arr[2]; i = arr[3]; }
  #endif
  FI XYZIval<T>          copy()                         const { return *this; }
  FI XYZIval<T>           ABS()                         const { return { T(_ABS(x)), T(_ABS(y)), T(_ABS(z)), T(_ABS(i)) }; }
  FI XYZIval<int16_t>   asInt()                               { return { int16_t(x), int16_t(y), int16_t(z), int16_t(i) }; }
  FI XYZIval<int16_t>   asInt()                         const { return { int16_t(x), int16_t(y), int16_t(z), int16_t(i) }; }
  FI XYZIval<int32_t>  asLong()                               { return { int32_t(x), int32_t(y), int32_t(z), int32_t(i) }; }
  FI XYZIval<int32_t>  asLong()                         const { return { int32_t(x), int32_t(y), int32_t(z), int32_t(i) }; }
  FI XYZIval<int32_t>  ROUNDL()                               { return { int32_t(LROUND(x)), int32_t(LROUND(y)), int32_t(LROUND(z)), int32_t(LROUND(i)) }; }
  FI XYZIval<int32_t>  ROUNDL()                         const { return { int32_t(LROUND(x)), int32_t(LROUND(y)), int32_t(LROUND(z)), int32_t(LROUND(i)) }; }
  FI XYZIval<float>   asFloat()                               { return {   float(x),   float(y),   float(z),   float(i) }; }
  FI XYZIval<float>   asFloat()                         const { return {   float(x),   float(y),   float(z),   float(i) }; }
  //FI XYZIval<float> reciprocal()                        const { return {  _RECIP(x),  _RECIP(y),  _RECIP(z),  _RECIP(i) }; }
  //FI XYZIval<float> asLogical()                         const { XYZIval<float> o = asFloat(); toLogical(o); return o; }
  //FI XYZIval<float>  asNative()                         const { XYZIval<float> o = asFloat(); toNative(o);  return o; }
  FI operator       XYval<T>&()                               { return *(XYval<T>*)this; }
  FI operator const XYval<T>&()                         const { return *(const XYval<T>*)this; }
  FI operator       XYZval<T>&()                              { return *(XYZval<T>*)this; }
  FI operator const XYZval<T>&()                        const { return *(const XYZval<T>*)this; }
  FI       T&    operator[](const int i)                      { return pos[i]; }
  FI const T&    operator[](const int i)                const { return pos[i]; }
  FI XYZIval<T>& operator= (const T v)                        { set(v, v, v, v); return *this; }
  FI XYZIval<T>& operator= (const XYval<T>   &rs)             { set(rs.x, rs.y); return *this; }
  FI XYZIval<T>& operator= (const XYZval<T>  &rs)             { set(rs.x, rs.y, rs.z); return *this; }
  FI XYZIval<T>  operator+ (const XYval<T>   &rs)       const { XYZIval<T> ls = *this; ls.x += rs.x; ls.y += rs.y;                             return ls; }
  FI XYZIval<T>  operator+ (const XYval<T>   &rs)             { XYZIval<T> ls = *this; ls.x += rs.x; ls.y += rs.y;                             return ls; }
  FI XYZIval<T>  operator- (const XYval<T>   &rs)       const { XYZIval<T> ls = *this; ls.x -= rs.x; ls.y -= rs.y;                             return ls; }
  FI XYZIval<T>  operator- (const XYval<T>   &rs)             { XYZIval<T> ls = *this; ls.x -= rs.x; ls.y -= rs.y;                             return ls; }
  FI XYZIval<T>  operator* (const XYval<T>   &rs)       const { XYZIval<T> ls = *this; ls.x *= rs.x; ls.y *= rs.y;                             return ls; }
  FI XYZIval<T>  operator* (const XYval<T>   &rs)             { XYZIval<T> ls = *this; ls.x *= rs.x; ls.y *= rs.y;                             return ls; }
  FI XYZIval<T>  operator/ (const XYval<T>   &rs)       const { XYZIval<T> ls = *this; ls.x /= rs.x; ls.y /= rs.y;                             return ls; }
  FI XYZIval<T>  operator/ (const XYval<T>   &rs)             { XYZIval<T> ls = *this; ls.x /= rs.x; ls.y /= rs.y;                             return ls; }
  FI XYZIval<T>  operator+ (const XYZval<T>  &rs)       const { XYZIval<T> ls = *this; ls.x += rs.x; ls.y += rs.y; ls.z += rs.z;               return ls; }
  FI XYZIval<T>  operator+ (const XYZval<T>  &rs)             { XYZIval<T> ls = *this; ls.x += rs.x; ls.y += rs.y; ls.z += rs.z;               return ls; }
  FI XYZIval<T>  operator- (const XYZval<T>  &rs)       const { XYZIval<T> ls = *this; ls.x -= rs.x; ls.y -= rs.y; ls.z -= rs.z;               return ls; }
  FI XYZIval<T>  operator- (const XYZval<T>  &rs)             { XYZIval<T> ls = *this; ls.x -= rs.x; ls.y -= rs.y; ls.z -= rs.z;               return ls; }
  FI XYZIval<T>  operator* (const XYZval<T>  &rs)       const { XYZIval<T> ls = *this; ls.x *= rs.x; ls.y *= rs.y; ls.z *= rs.z;               return ls; }
  FI XYZIval<T>  operator* (const XYZval<T>  &rs)             { XYZIval<T> ls = *this; ls.x *= rs.x; ls.y *= rs.y; ls.z *= rs.z;               return ls; }
  FI XYZIval<T>  operator/ (const XYZval<T>  &rs)       const { XYZIval<T> ls = *this; ls.x /= rs.x; ls.y /= rs.y; ls.z /= rs.z;               return ls; }
  FI XYZIval<T>  operator/ (const XYZval<T>  &rs)             { XYZIval<T> ls = *this; ls.x /= rs.x; ls.y /= rs.y; ls.z /= rs.z;               return ls; }
  FI XYZIval<T>  operator+ (const XYZIval<T> &rs)       const { XYZIval<T> ls = *this; ls.x += rs.x; ls.y += rs.y; ls.z += rs.z; ls.i += rs.i; return ls; }
  FI XYZIval<T>  operator+ (const XYZIval<T> &rs)             { XYZIval<T> ls = *this; ls.x += rs.x; ls.y += rs.y; ls.z += rs.z; ls.i += rs.i; return ls; }
  FI XYZIval<T>  operator- (const XYZIval<T> &rs)       const { XYZIval<T> ls = *this; ls.x -= rs.x; ls.y -= rs.y; ls.z -= rs.z; ls.i -= rs.i; return ls; }
  FI XYZIval<T>  operator- (const XYZIval<T> &rs)             { XYZIval<T> ls = *this; ls.x -= rs.x; ls.y -= rs.y; ls.z -= rs.z; ls.i -= rs.i; return ls; }
  FI XYZIval<T>  operator* (const XYZIval<T> &rs)       const { XYZIval<T> ls = *this; ls.x *= rs.x; ls.y *= rs.y; ls.z *= rs.z; ls.i *= rs.i; return ls; }
  FI XYZIval<T>  operator* (const XYZIval<T> &rs)             { XYZIval<T> ls = *this; ls.x *= rs.x; ls.y *= rs.y; ls.z *= rs.z; ls.i *= rs.i; return ls; }
  FI XYZIval<T>  operator/ (const XYZIval<T> &rs)       const { XYZIval<T> ls = *this; ls.x /= rs.x; ls.y /= rs.y; ls.z /= rs.z; ls.i /= rs.i; return ls; }
  FI XYZIval<T>  operator/ (const XYZIval<T> &rs)             { XYZIval<T> ls = *this; ls.x /= rs.x; ls.y /= rs.y; ls.z /= rs.z; ls.i /= rs.i; return ls; }
  FI XYZIval<T>  operator* (const float &v)             const { XYZIval<T> ls = *this; ls.x *= v;    ls.y *= v;    ls.z *= v;    ls.i *= v;    return ls; }
  FI XYZIval<T>  operator* (const float &v)                   { XYZIval<T> ls = *this; ls.x *= v;    ls.y *= v;    ls.z *= v;    ls.i *= v;    return ls; }
  FI XYZIval<T>  operator* (const int &v)               const { XYZIval<T> ls = *this; ls.x *= v;    ls.y *= v;    ls.z *= v;    ls.i *= v;    return ls; }
  FI XYZIval<T>  operator* (const int &v)                     { XYZIval<T> ls = *this; ls.x *= v;    ls.y *= v;    ls.z *= v;    ls.i *= v;    return ls; }
  FI XYZIval<T>  operator/ (const float &v)             const { XYZIval<T> ls = *this; ls.x /= v;    ls.y /= v;    ls.z /= v;    ls.i /= v;    return ls; }
  FI XYZIval<T>  operator/ (const float &v)                   { XYZIval<T> ls = *this; ls.x /= v;    ls.y /= v;    ls.z /= v;    ls.i /= v;    return ls; }
  FI XYZIval<T>  operator/ (const int &v)               const { XYZIval<T> ls = *this; ls.x /= v;    ls.y /= v;    ls.z /= v;    ls.i /= v;    return ls; }
  FI XYZIval<T>  operator/ (const int &v)                     { XYZIval<T> ls = *this; ls.x /= v;    ls.y /= v;    ls.z /= v;    ls.i /= v;    return ls; }
  FI XYZIval<T>  operator>>(const int &v)               const { XYZIval<T> ls = *this; _RS(ls.x);    _RS(ls.y);    _RS(ls.z);    _RS(ls.i);    return ls; }
  FI XYZIval<T>  operator>>(const int &v)                     { XYZIval<T> ls = *this; _RS(ls.x);    _RS(ls.y);    _RS(ls.z);    _RS(ls.i);    return ls; }
  FI XYZIval<T>  operator<<(const int &v)               const { XYZIval<T> ls = *this; _LS(ls.x);    _LS(ls.y);    _LS(ls.z);    _LS(ls.i);    return ls; }
  FI XYZIval<T>  operator<<(const int &v)                     { XYZIval<T> ls = *this; _LS(ls.x);    _LS(ls.y);    _LS(ls.z);    _LS(ls.i);    return ls; }
  FI XYZIval<T>& operator+=(const XYval<T>   &rs)             { x += rs.x; y += rs.y;                       return *this; }
  FI XYZIval<T>& operator-=(const XYval<T>   &rs)             { x -= rs.x; y -= rs.y;                       return *this; }
  FI XYZIval<T>& operator*=(const XYval<T>   &rs)             { x *= rs.x; y *= rs.y;                       return *this; }
  FI XYZIval<T>& operator/=(const XYval<T>   &rs)             { x /= rs.x; y /= rs.y;                       return *this; }
  FI XYZIval<T>& operator+=(const XYZval<T>  &rs)             { x += rs.x; y += rs.y; z += rs.z;            return *this; }
  FI XYZIval<T>& operator-=(const XYZval<T>  &rs)             { x -= rs.x; y -= rs.y; z -= rs.z;            return *this; }
  FI XYZIval<T>& operator*=(const XYZval<T>  &rs)             { x *= rs.x; y *= rs.y; z *= rs.z;            return *this; }
  FI XYZIval<T>& operator/=(const XYZval<T>  &rs)             { x /= rs.x; y /= rs.y; z /= rs.z;            return *this; }
  FI XYZIval<T>& operator+=(const XYZIval<T> &rs)             { x += rs.x; y += rs.y; z += rs.z; i += rs.i; return *this; }
  FI XYZIval<T>& operator-=(const XYZIval<T> &rs)             { x -= rs.x; y -= rs.y; z -= rs.z; i -= rs.i; return *this; }
  FI XYZIval<T>& operator*=(const XYZIval<T> &rs)             { x *= rs.x; y *= rs.y; z *= rs.z; i *= rs.i; return *this; }
  FI XYZIval<T>& operator/=(const XYZIval<T> &rs)             { x /= rs.x; y /= rs.y; z /= rs.z; i /= rs.i; return *this; }
  FI XYZIval<T>& operator*=(const T &v)                       { x *= v;    y *= v;    z *= v;    i *= v;    return *this; }
  FI XYZIval<T>& operator>>=(const int &v)                    { _RS(x);    _RS(y);    _RS(z);    _RS(i);    return *this; }
  FI XYZIval<T>& operator<<=(const int &v)                    { _LS(x);    _LS(y);    _LS(z);    _LS(i);    return *this; }
  FI bool        operator==(const XYZval<T>  &rs)             { return x == rs.x && y == rs.y && z == rs.z; }
  FI bool        operator!=(const XYZval<T>  &rs)             { return !operator==(rs); }
  FI bool        operator==(const XYZval<T>  &rs)       const { return x == rs.x && y == rs.y && z == rs.z; }
  FI bool        operator!=(const XYZval<T>  &rs)       const { return !operator==(rs); }
  FI       XYZIval<T> operator-()                             { return { -x, -y, -z, -i }; }
  FI const XYZIval<T> operator-()                       const { return { -x, -y, -z, -i }; }
};
...
struct XYZIEval {
  ...
  FI T magnitude()                                      const { return (T)sqrtf(x*x + y*y + z*z + i*i + e*e); }
  FI operator T* ()                                           { return pos; }
  FI operator bool()                                          { return e || i || z || x || y; }
  FI void set(const T px)                                     { x = px;                                        }
  FI void set(const T px, const T py)                         { x = px;     y = py;                            }
  FI void set(const T px, const T py, const T pz)             { x = px;     y = py;     z = pz;                }
  FI void set(const T px, const T py, const T pz, const T pi) { x = px;     y = py;     z = pz;     i = pi;    }
  ...
  FI void set(const XYZval<T> pxyz)                           { x = pxyz.x; y = pxyz.y; z = pxyz.z;            }
  FI void set(const XYval<T> pxy, const T pz, const T pi)     { x = pxy.x;  y = pxy.y;  z = pz;     i = pi;    }
  FI void set(const XYval<T> pxy, const T pz, const T pi, const T pe)     { x = pxy.x;  y = pxy.y;  z = pz;     i = pi;   e = pe;    }
  FI void set(const XYval<T> pxy, const XYval<T> pzi)         { x = pxy.x;  y = pxy.y;  z = pzi.z;  i = pzi.i; }
  ...
  FI void set(const T (&arr)[XYZ])                            { x = arr[0]; y = arr[1]; z = arr[2]; }
  FI void set(const T (&arr)[XYZI])                           { x = arr[0]; y = arr[1]; z = arr[2]; i = arr[3]; }
  FI void set(const T (&arr)[XYZIE])                           { x = arr[0]; y = arr[1]; z = arr[2]; i = arr[3]; e=arr[4]; }
  
  #if XYZIE_N > XYZIE
    FI void set(const T (&arr)[XYZIE_N])                       { x = arr[0]; y = arr[1]; z = arr[2]; i = arr[3]; e= arr[4] }
  #endif
  FI XYZIEval<T>          copy()                         const { return *this; }
  FI XYZIEval<T>           ABS()                         const { return { T(_ABS(x)), T(_ABS(y)), T(_ABS(z)), T(_ABS(i)), T(_ABS(e)) }; }
  FI XYZIEval<int16_t>   asInt()                               { return { int16_t(x), int16_t(y), int16_t(z), int16_t(i) , int16_t(e) }; }
  FI XYZIEval<int16_t>   asInt()                         const { return { int16_t(x), int16_t(y), int16_t(z), int16_t(i), int16_t(e) }; }
  FI XYZIEval<int32_t>  asLong()                               { return { int32_t(x), int32_t(y), int32_t(z), int32_t(i), int32_t(e) }; }
  FI XYZIEval<int32_t>  asLong()                         const { return { int32_t(x), int32_t(y), int32_t(z), int32_t(i), int32_t(e) }; }
  FI XYZIEval<int32_t>  ROUNDL()                               { return { int32_t(LROUND(x)), int32_t(LROUND(y)), int32_t(LROUND(z)), int32_t(LROUND(i)), int32_t(LROUND(e)) }; }
  FI XYZIEval<int32_t>  ROUNDL()                         const { return { int32_t(LROUND(x)), int32_t(LROUND(y)), int32_t(LROUND(z)), int32_t(LROUND(i)), int32_t(LROUND(e)) }; }
  FI XYZIEval<float>   asFloat()                               { return {   float(x),   float(y),   float(z),   float(i),   float(e) }; }
  FI XYZIEval<float>   asFloat()                         const { return {   float(x),   float(y),   float(z),   float(i),   float(e) }; }
  //FI XYZIval<float> reciprocal()                        const { return {  _RECIP(x),  _RECIP(y),  _RECIP(z),  _RECIP(i) }; }
  //FI XYZIval<float> asLogical()                         const { XYZIval<float> o = asFloat(); toLogical(o); return o; }
  //FI XYZIval<float>  asNative()                         const { XYZIval<float> o = asFloat(); toNative(o);  return o; }
  FI operator       XYval<T>&()                               { return *(XYval<T>*)this; }
  ...
  FI       T&    operator[](const int i)                      { return pos[i]; }
  FI const T&    operator[](const int i)                const { return pos[i]; }
  FI XYZIEval<T>& operator= (const T v)                        { set(v, v, v, v, v); return *this; }
  FI XYZIEval<T>& operator= (const XYval<T>   &rs)             { set(rs.x, rs.y); return *this; }
  FI XYZIEval<T>& operator= (const XYZval<T>  &rs)             { set(rs.x, rs.y, rs.z); return *this; }
  FI XYZIEval<T>  operator+ (const XYval<T>   &rs)       const { XYZIEval<T> ls = *this; ls.x += rs.x; ls.y += rs.y;                             return ls; }
  FI XYZIEval<T>  operator+ (const XYval<T>   &rs)             { XYZIEval<T> ls = *this; ls.x += rs.x; ls.y += rs.y;                             return ls; }
  FI XYZIEval<T>  operator- (const XYval<T>   &rs)       const { XYZIEval<T> ls = *this; ls.x -= rs.x; ls.y -= rs.y;                             return ls; }
  FI XYZIEval<T>  operator- (const XYval<T>   &rs)             { XYZIEval<T> ls = *this; ls.x -= rs.x; ls.y -= rs.y;                             return ls; }
  FI XYZIEval<T>  operator* (const XYval<T>   &rs)       const { XYZIEval<T> ls = *this; ls.x *= rs.x; ls.y *= rs.y;                             return ls; }
  FI XYZIEval<T>  operator* (const XYval<T>   &rs)             { XYZIEval<T> ls = *this; ls.x *= rs.x; ls.y *= rs.y;                             return ls; }
  FI XYZIEval<T>  operator/ (const XYval<T>   &rs)       const { XYZIEval<T> ls = *this; ls.x /= rs.x; ls.y /= rs.y;                             return ls; }
  FI XYZIEval<T>  operator/ (const XYval<T>   &rs)             { XYZIEval<T> ls = *this; ls.x /= rs.x; ls.y /= rs.y;                             return ls; }
  FI XYZIEval<T>  operator+ (const XYZval<T>  &rs)       const { XYZIEval<T> ls = *this; ls.x += rs.x; ls.y += rs.y; ls.z += rs.z;               return ls; }
  FI XYZIEval<T>  operator+ (const XYZval<T>  &rs)             { XYZIEval<T> ls = *this; ls.x += rs.x; ls.y += rs.y; ls.z += rs.z;               return ls; }
  FI XYZIEval<T>  operator- (const XYZval<T>  &rs)       const { XYZIEval<T> ls = *this; ls.x -= rs.x; ls.y -= rs.y; ls.z -= rs.z;               return ls; }
  FI XYZIEval<T>  operator- (const XYZval<T>  &rs)             { XYZIEval<T> ls = *this; ls.x -= rs.x; ls.y -= rs.y; ls.z -= rs.z;               return ls; }
  FI XYZIEval<T>  operator* (const XYZval<T>  &rs)       const { XYZIEval<T> ls = *this; ls.x *= rs.x; ls.y *= rs.y; ls.z *= rs.z;               return ls; }
  FI XYZIEval<T>  operator* (const XYZval<T>  &rs)             { XYZIEval<T> ls = *this; ls.x *= rs.x; ls.y *= rs.y; ls.z *= rs.z;               return ls; }
  FI XYZIEval<T>  operator/ (const XYZval<T>  &rs)       const { XYZIEval<T> ls = *this; ls.x /= rs.x; ls.y /= rs.y; ls.z /= rs.z;               return ls; }
  FI XYZIEval<T>  operator/ (const XYZval<T>  &rs)             { XYZIEval<T> ls = *this; ls.x /= rs.x; ls.y /= rs.y; ls.z /= rs.z;               return ls; }
  FI XYZIEval<T>  operator+ (const XYZIEval<T> &rs)       const { XYZIEval<T> ls = *this; ls.x += rs.x; ls.y += rs.y; ls.z += rs.z; ls.i += rs.i; ls.e += rs.e; return ls; }
  FI XYZIEval<T>  operator+ (const XYZIEval<T> &rs)             { XYZIEval<T> ls = *this; ls.x += rs.x; ls.y += rs.y; ls.z += rs.z; ls.i += rs.i; ls.e += rs.e; return ls; }
  FI XYZIEval<T>  operator- (const XYZIEval<T> &rs)       const { XYZIEval<T> ls = *this; ls.x -= rs.x; ls.y -= rs.y; ls.z -= rs.z; ls.i -= rs.i; ls.e -= rs.e; return ls; }
  FI XYZIEval<T>  operator- (const XYZIEval<T> &rs)             { XYZIEval<T> ls = *this; ls.x -= rs.x; ls.y -= rs.y; ls.z -= rs.z; ls.i -= rs.i; ls.e -= rs.e; return ls; }
  FI XYZIEval<T>  operator* (const XYZIEval<T> &rs)       const { XYZIEval<T> ls = *this; ls.x *= rs.x; ls.y *= rs.y; ls.z *= rs.z; ls.i *= rs.i; ls.e -= rs.e; return ls; }
  FI XYZIEval<T>  operator* (const XYZIEval<T> &rs)             { XYZIEval<T> ls = *this; ls.x *= rs.x; ls.y *= rs.y; ls.z *= rs.z; ls.i *= rs.i; ls.e *= rs.e; return ls; }
  FI XYZIEval<T>  operator/ (const XYZIEval<T> &rs)       const { XYZIEval<T> ls = *this; ls.x /= rs.x; ls.y /= rs.y; ls.z /= rs.z; ls.i /= rs.i; ls.e /= rs.e; return ls; }
  FI XYZIEval<T>  operator/ (const XYZIEval<T> &rs)             { XYZIEval<T> ls = *this; ls.x /= rs.x; ls.y /= rs.y; ls.z /= rs.z; ls.i /= rs.i; ls.e /= rs.e; return ls; }
  FI XYZIEval<T>  operator* (const float &v)             const { XYZIEval<T> ls = *this; ls.x *= v;    ls.y *= v;    ls.z *= v;    ls.i *= v; ls.e *= v;    return ls; }
  FI XYZIEval<T>  operator* (const float &v)                   { XYZIEval<T> ls = *this; ls.x *= v;    ls.y *= v;    ls.z *= v;    ls.i *= v; ls.e *= v;   return ls; }
  FI XYZIEval<T>  operator* (const int &v)               const { XYZIEval<T> ls = *this; ls.x *= v;    ls.y *= v;    ls.z *= v;    ls.i *= v; ls.e *= v;  return ls; }
  FI XYZIEval<T>  operator* (const int &v)                     { XYZIEval<T> ls = *this; ls.x *= v;    ls.y *= v;    ls.z *= v;    ls.i *= v; ls.e *= v;   return ls; }
  FI XYZIEval<T>  operator/ (const float &v)             const { XYZIEval<T> ls = *this; ls.x /= v;    ls.y /= v;    ls.z /= v;    ls.i /= v; ls.e /= v;   return ls; }
  FI XYZIEval<T>  operator/ (const float &v)                   { XYZIEval<T> ls = *this; ls.x /= v;    ls.y /= v;    ls.z /= v;    ls.i /= v; ls.e /= v;   return ls; }
  FI XYZIEval<T>  operator/ (const int &v)               const { XYZIEval<T> ls = *this; ls.x /= v;    ls.y /= v;    ls.z /= v;    ls.i /= v; ls.e /= v;   return ls; }
  FI XYZIEval<T>  operator/ (const int &v)                     { XYZIEval<T> ls = *this; ls.x /= v;    ls.y /= v;    ls.z /= v;    ls.i /= v; ls.e /= v;   return ls; }
  FI XYZIEval<T>  operator>>(const int &v)               const { XYZIEval<T> ls = *this; _RS(ls.x);    _RS(ls.y);    _RS(ls.z);    _RS(ls.i); _RS(ls.e);    return ls; }
  FI XYZIEval<T>  operator>>(const int &v)                     { XYZIEval<T> ls = *this; _RS(ls.x);    _RS(ls.y);    _RS(ls.z);    _RS(ls.i); _RS(ls.e);   return ls; }
  FI XYZIEval<T>  operator<<(const int &v)               const { XYZIEval<T> ls = *this; _LS(ls.x);    _LS(ls.y);    _LS(ls.z);    _LS(ls.i); _LS(ls.e);   return ls; }
  FI XYZIEval<T>  operator<<(const int &v)                     { XYZIEval<T> ls = *this; _LS(ls.x);    _LS(ls.y);    _LS(ls.z);    _LS(ls.i); _LS(ls.e);   return ls; }
  FI XYZIEval<T>& operator+=(const XYval<T>   &rs)             { x += rs.x; y += rs.y;                       return *this; }
  FI XYZIEval<T>& operator-=(const XYval<T>   &rs)             { x -= rs.x; y -= rs.y;                       return *this; }
  FI XYZIEval<T>& operator*=(const XYval<T>   &rs)             { x *= rs.x; y *= rs.y;                       return *this; }
  FI XYZIEval<T>& operator/=(const XYval<T>   &rs)             { x /= rs.x; y /= rs.y;                       return *this; }
  FI XYZIEval<T>& operator+=(const XYZval<T>  &rs)             { x += rs.x; y += rs.y; z += rs.z;            return *this; }
  FI XYZIEval<T>& operator-=(const XYZval<T>  &rs)             { x -= rs.x; y -= rs.y; z -= rs.z;            return *this; }
  FI XYZIEval<T>& operator*=(const XYZval<T>  &rs)             { x *= rs.x; y *= rs.y; z *= rs.z;            return *this; }
  FI XYZIEval<T>& operator/=(const XYZval<T>  &rs)             { x /= rs.x; y /= rs.y; z /= rs.z;            return *this; }
  FI XYZIEval<T>& operator+=(const XYZIEval<T> &rs)             { x += rs.x; y += rs.y; z += rs.z; i += rs.i; i += rs.e; return *this; }
  FI XYZIEval<T>& operator-=(const XYZIEval<T> &rs)             { x -= rs.x; y -= rs.y; z -= rs.z; i -= rs.i; i -= rs.e; return *this; }
  FI XYZIEval<T>& operator*=(const XYZIEval<T> &rs)             { x *= rs.x; y *= rs.y; z *= rs.z; i *= rs.i; i *= rs.e; return *this; }
  FI XYZIEval<T>& operator/=(const XYZIEval<T> &rs)             { x /= rs.x; y /= rs.y; z /= rs.z; i /= rs.i; i /= rs.e; return *this; }
  FI XYZIEval<T>& operator*=(const T &v)                       { x *= v;    y *= v;    z *= v;    i *= v;  e *= v;    return *this; }
  FI XYZIEval<T>& operator>>=(const int &v)                    { _RS(x);    _RS(y);    _RS(z);    _RS(i); _RS(e);   return *this; }
  FI XYZIEval<T>& operator<<=(const int &v)                    { _LS(x);    _LS(y);    _LS(z);    _LS(i); _LS(e);    return *this; }
  FI bool        operator==(const XYZval<T>  &rs)             { return x == rs.x && y == rs.y && z == rs.z; }
  FI bool        operator!=(const XYZval<T>  &rs)             { return !operator==(rs); }
  FI bool        operator==(const XYZval<T>  &rs)       const { return x == rs.x && y == rs.y && z == rs.z; }
  FI bool        operator!=(const XYZval<T>  &rs)       const { return !operator==(rs); }
  FI       XYZIEval<T> operator-()                             { return { -x, -y, -z, -i, -e}; }
  FI const XYZIEval<T> operator-()                       const { return { -x, -y, -z, -i, -e }; }
};
...
const xyzie_char_t axis_codes { 'X', 'Y', 'Z', 'I', 'E'}; 
//checker si en replacant on peut get les bon axis codes
